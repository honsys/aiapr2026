...
\item
  ...
  \texttt{cython.build()}, \texttt{go.run()}, and more.
\item
  \textbf{Python \& Cython Compilation}: \texttt{python.compile()}
  byte-compiles \texttt{.py} → \texttt{.pyc}; \texttt{cython.build()}
  transpiles \texttt{.pyx} → C → native \texttt{.so} shared library for
  maximum performance.
\item
...
\subsubsection{11. ASCII Art \& SVG}\label{ascii-art-svg}
...
\item
  ...
  builtins --- run Python scripts natively, byte-compile \texttt{.py} →
  \texttt{.pyc}, full Cython pipeline \texttt{.pyx} → C → \texttt{.so}
  native extension, pip management.
\end{itemize}

\subsubsection{14. Heterogeneous Arrays \&
Dictionaries}\label{heterogeneous-arrays-dictionaries}
...
comparisons) - \texttt{sys.helpfull()} --- write complete reference to
\texttt{helpfull\_DATETIME.md}, open in system viewer; returns filename
- \texttt{sys.helpless()} --- same as helpfull but pipes through
\texttt{less} instead of opening viewer; returns filename -
\texttt{sys.exit()} --- exit interpreter -
...
\item
  \texttt{fin.greeks(type,\ strike,\ spot,\ rate,\ vol,\ t)} → NPV,
  Delta, Gamma, Theta, Vega
\item
  \texttt{fin.date(d,m,y)}, \texttt{fin.calendar(name)},
  \texttt{fin.is\_holiday(cal,d,m,y)}, \texttt{fin.add\_days(...)},
  \texttt{fin.diff\_days(...)}
\end{itemize}
...
\texttt{algo.array({[}d1,d2,...{]},\ {[}fill{]})} --- allocate N-D
array; fill default nil - \texttt{algo.array\_get(arr,\ i0,\ i1,\ ...)}
--- index into nested array -
\texttt{algo.array\_set(arr,\ val,\ i0,\ i1,\ ...)} --- set element
in-place → bool - \texttt{algo.array\_push(arr,\ val)} --- append to 1-D
array → bool - \texttt{algo.array\_len(arr)} --- outermost dimension
length → double - \texttt{algo.array\_shape(arr)} --- dimensions as
numeric vector

Dictionaries (arbitrary key/value tuples): -
\texttt{algo.dict({[}k,v,...{]})} --- create dict, optional initial
pairs - \texttt{algo.dict\_set(d,\ key,\ val)} --- set key (mutates
in-place) → bool - \texttt{algo.dict\_get(d,\ key)} --- get value (nil
if missing) - \texttt{algo.dict\_del(d,\ key)} --- remove key → bool -
\texttt{algo.dict\_has(d,\ key)} --- existence check → bool -
\texttt{algo.dict\_keys(d)} --- all keys → string vector -
\texttt{algo.dict\_vals(d)} --- all values → heterogeneous array -
\texttt{algo.dict\_len(d)} --- entry count → double
...
\tightlist
\item
  \texttt{tcp.listen(port)}, \texttt{tcp.accept(fd)},
  \texttt{tcp.connect(host,\ port)} --- socket management
\item
  \texttt{tcp.send(fd,\ msg)}, \texttt{tcp.recv(fd,\ {[}size{]})},
  \texttt{tcp.close(fd)} --- data transfer
\item
  \texttt{tcp.nic()} --- network interface configurations
\item
  \texttt{tcp.routes()} --- system routing table
\end{itemize}
...
\item
  \textbf{\texttt{python}} --- Python 3 polyglot:
  \texttt{python.run(file\_or\_code)} · \texttt{python.compile(file)}
  byte-compiles \texttt{.py}→\texttt{.pyc} · \texttt{python.pip(pkg)}
  installs packages. C++=\texttt{PyRun\_SimpleFile} ·
  Julia=\texttt{PyCall.jl} · Rust=\texttt{pyo3}
\item
  \textbf{\texttt{cython}} --- Cython polyglot:
  \texttt{cython.compile(pyx)} transpiles \texttt{.pyx}→\texttt{.c} ·
  \texttt{cython.build(pyx)} full pipeline to \texttt{.so} ·
  \texttt{cython.run(pyx)} compile+execute · \texttt{cython.pip(pkg)}.
  Pipeline:
  \texttt{.pyx\ →\ cython\ -3\ →\ .c\ →\ gcc\ -shared\ -fPIC\ →\ .so}.
  C++=\texttt{direct} · Python=\texttt{cythonize()} ·
  Rust=\texttt{PyO3+maturin}
\item
