...
\subsubsection{container --- Docker \& Kubernetes}\label{container-docker-kubernetes}

\begin{itemize}
\tightlist
\item \texttt{container.docker\_run(image, cmd)}, \texttt{docker\_ps()}, \texttt{docker\_build(path, tag)}, \texttt{docker\_stop(id)}
\item \texttt{container.k3s\_apply(yaml)}, \texttt{k3s\_get(resource)}, \texttt{k3s\_pods()}, \texttt{k3s\_nodes()}, \texttt{k3s\_logs(pod)}
\end{itemize}
