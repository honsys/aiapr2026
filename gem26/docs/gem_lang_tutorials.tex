...
\item
  ...
  \texttt{mobl}, \texttt{astro}, \texttt{nlp} built into the runtime.
\item
...
\NormalTok{  sys, math, ai, text, rex, algo, bev, file, zip, nlp, img, geo, mobl,}
\NormalTok{  cpp, tcp, itr, data, container, vm, go, ruby, node, rust,}
\NormalTok{  fin, bsm, chart, astro, drvr}
...
\NormalTok{  use "mobl.gm" then: mobl phone("name")  phone.dictate(text)  phone.make\_feature(lat,lon,text)}
...
\item \textbf{container}: Docker (\texttt{docker\_run/ps/build/stop}) + K3s (\texttt{k3s\_apply/get/pods/nodes/logs})
...
Uses \texttt{mobl.gm} + \texttt{travel\_log.html} (PWA). Voice dictation
